...
public class UserCloudantAdapter: UserAdapter {
    let client: CouchDBClient
    var database: Database {
        // TODO: Properly ensure database exists
        client.createDB("user") { _, _ in }
        return client.database("user")
    }
    ...
    public func findAll(onCompletion: @escaping ([User], Swift.Error?) -> Void) {
        database.retrieveAll(includeDocuments: true) { maybeDocuments, error in
            ...
            do {
                let models = try documents.map { _, document -> User in
                    ...
                    let model = try User(json: modelJSON)
    ...
    public func create(_ model: User, onCompletion: @escaping (User?, Swift.Error?) -> Void) {
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (User?, Swift.Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: User, onCompletion: @escaping (User?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (User?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
